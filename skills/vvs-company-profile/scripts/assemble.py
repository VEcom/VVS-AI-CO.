"""vvs-company-profile L1 회사소개서 — 조립 스크립트.
...
_TEMPLATE = Path(__file__).resolve().parents[1] / "templates" / "company_profile.blocks.json"
...
def build_blocks(data: dict) -> list:
    """입력 데이터를 block_library 블록 스펙으로 변환.

    반환: [(block_id, data_dict), ...] — 템플릿이 정의한 순서대로.
    """
    data = data or {}
    field_map = {
        "header": {"title": data.get("title", "회사소개서"), "date": data.get("date", "")},
        "company_intro": {
