"""vvs-vendor-dossier L2 공사지명원 — 조립 스크립트.
...
_TEMPLATE = Path(__file__).resolve().parents[1] / "templates" / "vendor_dossier.blocks.json"
...
def build_blocks(data: dict) -> list:
    ...
    data = data or {}
    field_map = {
        "header": {"title": data.get("title", "공사지명원"), "date": data.get("date", "")},
        "company_intro": {
